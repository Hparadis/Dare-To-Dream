...
const Camping = ({ groupId, currentUser }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [results, setResults] = useState([]);
  const [members, setMembers] = useState([]);
  const [things, setThings] = useState(["Tent", "Sleeping bag"]);

  const handleSearch = () => {
    setResults([
      {
        id: 1,
        name: "National Forest Campground",
        type: "Public",
        conditions: "Permit required",
      },
      {
        id: 2,
        name: "Lakeview Camping Site",
        type: "Private",
        conditions: "Booking required",
      },
    ]);
  };
